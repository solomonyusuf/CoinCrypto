          ...
          <div class="col-md-4 col-sm-12 border-bottom mb-3">
              <a href="{{ route('article_detail', [$data->slug, $data->id]) }}" class="text-decoration-none">
                  <div class="card border-0 shadow-sm p-3 d-flex flex-row align-items-center gap-3">
                      ...
                      <!-- Article Content -->
                      <div class="flex-grow-1">
                          <h6 class="fw-semibold fs-5 text-dark mb-1">{{ $data->title }}</h6>
                          
                          <!-- Author List -->
                          <div class="mt-2">
                              <span class="text-uppercase fw-semibold text-muted small">
                                  By 
                                  {{ implode(', ', $data->article_creators->map(fn($creator) => $creator->user->first_name . ' ' . $creator->user->last_name)->toArray()) }}
                              </span>
                          </div>
                      </div>
                  </div>
