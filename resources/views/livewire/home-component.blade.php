       ...
       @foreach ($latests as $latest)
          ...
          <div class="col-lg-6 pe-lg-0 mb-3">
            ...
            <div class="blog-bg d-flex flex-column justify-content-between p-9 h-100 rounded-start-3 flex-grow-1">
              <img src="{{ $latest?->article_creators->where(['originator'=> true])->first()?->user?->image ?? $latest?->article_creators->first()?->user?->image}}" alt="user" width="44" height="44" class="rounded-circle">
            </div>
          </a>
          </div>
            ...
            <div class="d-flex flex-column justify-content-between h-100">
              <div class="d-flex align-items-start justify-content-between">
                <div class="position-relative" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="author">
                  <img src="{{ $data?->article_creators->where(['originator'=> true])->first()?->user?->image }}" class="rounded-circle img-fluid" width="40" height="40">
                </div>
                <span class="badge text-bg-primary fs-2 fw-semibold">{{ $data->article_category?->title}}</span>
              </div>
              <div>
                <a href="{{ route('article_detail', [$data->slug, $data->id]) }}" class="fs-7 my-4 fw-semibold text-white d-block lh-sm text-primary">
                {{ $data->title }} 
                </a>
      ...
      <div class="col-md-6 col-lg-4">
        <div class="card overflow-hidden hover-img">
          <div class="position-relative">
            ...
            <img src="{{ $data?->article_creators->where(['originator'=> true])->first()?->user?->image ?? $latest?->article_creators->first()?->user?->image }}" alt="modernize-img" class="img-fluid rounded-circle position-absolute bottom-0 start-0 mb-n9 ms-9" width="40" height="40" data-bs-toggle="tooltip" data-bs-placement="top" >
          </div>
