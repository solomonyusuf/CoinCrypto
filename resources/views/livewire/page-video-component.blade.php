    ...
    <div class="w-full pt-8 pb-[64px] bottom-0 flex flex-col items-center">
      ...
      <div
        ...
        <div class="flex gap-6 w-full lgmax:flex-col lg:flex-row mb-8">
          <div class="flex flex-1 flex-col">
            <div class="font-title {{ $setting->theme == 'white' ? 'text-color-dark-grey text-charcoal-600' : 'text-color-white' }} mb-6 uppercase">
              About
            </div>
            <div class="font-body-lg flex flex-col gap-6 {{ $setting->theme == 'white' ? 'text-color-dark-grey' : 'text-color-white' }}">
              <div>
      ...
      <div class="container mx-auto xs:p-4 xs:grid-cols-4 xs:gap-4 sm:p-4 sm:grid-cols-4 sm:gap-4 md:p-6 md:grid-cols-8 md:gap-6 lg:p-6 lg:grid-cols-12 lg:gap-6 xl:p-6 xl:grid-cols-16 xl:gap-6">
        <div  class="{{ $setting->theme == 'white' ? 'bg-white' : 'bg-black'  }} flex flex-wrap flex-col flex-start container-desktop-large mt-8 gap-6 content-card-section">
          <h2
            class="w-full flex flex-row align-middle justify-start border-t-[1px] border-charcoal-600 pt-6 uppercase font-title {{ $setting->theme == 'white' ? 'text-color-dark-grey text-charcoal-900' : 'text-color-white' }}">
            {{ $category->title }} Series</h2>
          <div class="flex flex-row justify-start gap-6 content-card-section mdmax:overflow-x-auto pb-1">
            @foreach ($videos as $data)
            <div class="{{ $setting->theme == 'white' ? 'bg-white' : 'bg-black'  }} flex gap-6 flex-col  content-card-image--video-container"><a
              ...
              <div class="relative visible content-card-image--videoseries mdmax:top-3">
                ...
                <div class="absolute left-1 flex items-center bg-[#1B2329] py-[2px] pl-[4px] pr-[8px] rounded z-[5] w-[110px] justify-start"
                  style="bottom: 4px;"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none"
                    ...
                    <g mask="url(#a)">
                      <path fill="{{ $setting->theme == 'white' ? '#F8BF1E' : 'white' }}"
            ...
            <div class="flex flex-col h-auto mdmax:flex-shrink"><a
                class="md:mb-4 mdmax:mb-1 hover:underline {{ $setting->theme == 'white' ? 'text-color-dark-grey text-charcoal-900' : 'text-color-white' }}"
                href="{{ route('video_detail', $data->category_id).'?video='.$data->id }}">
                <h3 class="font-headline-2xs font-medium {{ $setting->theme == 'white' ? 'text-color-dark-grey text-charcoal-900' : 'text-color-white' }}">
                {{ $data->title }}
                </h3>
              </a>
