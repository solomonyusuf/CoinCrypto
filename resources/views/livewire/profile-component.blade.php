<div>
  <div class="max-w-2xl mx-auto mt-4 mb-4 bg-white shadow-lg rounded-lg p-6">
    <form enctype="multipart/form-data" action="{{ route('update_profile') }}" method="post" class="space-y-6">
        @csrf
        ...
        <div class="space-y-4">
            ...
            @if($data->image)
            <div class="mb-3">
                <img src="{{ asset($data->image) }}" class="h-16 w-16 rounded-full object-cover" />
            </div>
            @endif
            ...
            <div>
                ...
                <input name="first_name" value="{{ $data->first_name }}" type="text" class="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            </div>

            <!-- Last Name -->
            <div>
                <label class="block text-gray-700 font-medium mb-1">Last Name *</label>
                <input name="last_name" value="{{ $data->last_name }}" type="text" class="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            </div>

            <!-- Info -->
            <div>
                <label class="block text-gray-700 font-medium mb-1">Info</label>
                <textarea name="info" class="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{!! $data->info !!}</textarea>
            </div>

            <!-- Email (Disabled) -->
            <div>
                <label class="block text-gray-700 font-medium mb-1">Email *</label>
                <input name="email" value="{{ $data->email }}" type="email" disabled class="w-full p-2 border border-gray-300 rounded-lg bg-gray-100 text-gray-500 cursor-not-allowed">
            </div>
